...
type itable_t
    ...
    contains
        ...
        procedure :: sort     => itbl_sort    
        procedure :: get_size => itbl_get_size    
...
subroutine itbl_sort(this)
    !! Sorts all the rows of the table in ascending order.

    class(itable_t), target, intent(in out) :: this
    integer, dimension(:), pointer :: row_ptr
    integer :: irow, ibeg, iend 

    do irow = 1, this%num_rows
        row_ptr => null()
        ibeg = this%row_indx(irow)
        iend = this%row_indx(irow+1) - 1
        if (iend > ibeg) then
            call this%buffer%get_data(row_ptr, ibeg, iend)
            call iqsort(row_ptr)
        end if
    end do
...
function itbl_get_size(this) result(res)
